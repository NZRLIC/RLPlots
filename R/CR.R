...
#' @param CROptions catch rate options
#' @param PlotOptions plot options
#' @export
#' 
CR <- function(stock, source.dir, target.dir = source.dir, CROptions = .CROptions,
               PlotOptions = .PlotOptions)
{
  ...
  { 
    PlotType(paste(target.dir, "/", stock, "CRObsPred", sep = ""), PlotOptions, width = 170, height = 200)
...
# vh : comment out followng line cause it doens't work with 2 periods/yr
   #for(p in prange1) lines(c(LB,UB)~c(p,p),data=cr[cr$x==p,],pch="-",type="o",lwd=thin)
    ...
    {
      mtext(paste(source.dir," ",stock,": Observed and predicted for CR fits."),side=1,line=-0.7,outer=TRUE,cex=0.7)
  ...
  if( CROptions$Resid )
  {
    PlotType(paste(target.dir, "/", stock, "CRResid", sep = ""), PlotOptions, width = 170, height = 200)
    ...
    if ( PlotOptions$Captions ) 
    {
      mtext(paste(source.dir," ",stock,": Standardised residual for CR fits"),side=1,line=-0.7,outer=TRUE,cex=0.7)
    }
    dev.off()

    #Resids v predicted
    PlotType(paste(target.dir, "/", stock, "CRResidPred", sep = ""), PlotOptions, width = 170, height = 200)
    plot(StdRes~Pred,data=cr,pch=16,lwd=PlotOptions$thin,ylab="Standardised residual",xlab="Predicted")
    abline(h=0,lty=2)
    if ( PlotOptions$Captions ) 
    {
      mtext(paste(source.dir," ",stock,": Standardised residual for CR fits"),side=1,line=-0.7,outer=TRUE,cex=0.7)
    ...
    #qq plot
    PlotType(paste(target.dir, "/", stock, "CRQQ", sep = ""), PlotOptions, width = 170, height = 200)
