...
LFzero <- function(stock,source.dir,target.dir = source.dir)
{
  ...
  par(las=1,oma=c(1,1,1,1),mar=c(4,4.5,1,1),mgp=c(3.5,1,0), cex = 3)
  #plot
  ...
  legend("topright",legend=c("Males","Immature females","Mature females"),lwd=c(PlotOptions$thin,PlotOptions$thin,PlotOptions$thick),lty=c(1,2,2),bty="n",cex=1)  #  AS edit : replaced legend coordinates "70,max(lfzero$Nmale,lfzero$Nfemale,lfzero$Nfemmat)/1000"
  mtext("Size (mm TW)",1,2, cex = 3)
  #Texts
  if ( PlotOptions$Captions ) mtext(paste(source.dir," ",stock,": Initial length structure"),side=1,line=0,outer=TRUE,cex=2)
  dev.off()
}
