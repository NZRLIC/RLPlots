...
Bio <- function(stock, source.dir, target.dir = source.dir)
{
  ...
  if( BioOptions$VulnB )
  {
    ...
    legend("top",legend=c("AW","SS"),lwd=c(PlotOptions$thin, PlotOptions$thick), bty = "n", cex = 0.8) ## AS edit
    #mtext(xlab,1,2)
    if ( PlotOptions$Captions ) mtext(paste(source.dir," ",stock,": Vulnerable Biomass"),side=1,line=0,outer=TRUE,cex=0.7)
    dev.off()
  }
  ...
  if ( BioOptions$RecB )
  {
    ...
    for ( Season in 1:2 )
    {
      ...
      if( Season == 1 )
      {
          mtext("Recruited biomass (thousands t)", side = 2, line = 3)
          legend("top",
                 ...
                 lty = c(1,2,2,1), bty = "n", cex = 0.8)
      }
      mtext(c("AW","SS")[Season], side = 3, line = 0.1, adj = 0)
    }
  ...
  if ( BioOptions$TotalRecB )
  {
    ...
    for ( Season in 1:2 )
    {
      ...
      if ( Season == 1 )
      {
        mtext("Total biomass (thousands t)", side = 2, line = 3)
        legend("top", lty = c(1,2,2,1), bty = "n", cex = 0.8,
               legend = c("Males","Immature females","Mature females","Total"),
               lwd = c(PlotOptions$thin, PlotOptions$thin, PlotOptions$thick, PlotOptions$thick))
      }
      mtext(c("AW","SS")[Season], side = 3, line = 0.1, adj = 0)
